...
class TestCharacterStore(unittest.TestCase):
    ...
    def test_load_scout_json(self) -> None:
        scout_path = self.data_dir / "scout.json"
        self.assertTrue(scout_path.is_file(), f"missing sample file: {scout_path}")

        character = CharacterStore().load(scout_path)
        self.assertEqual(character.id, "scout")
        ...
        attack = character.get_attack("blade")
        ...
        self.assertEqual(attack.components[1].option_id, "bonus_strike")
...
class TestCharacterEngineIntegration(unittest.TestCase):
    ...
    def test_roll_character_attack_with_bonus_option(self) -> None:
    ...
    def test_warrior_reroll_low_appends_reroll_ops(self) -> None:
        warrior = self.store.get("warrior")
        roll_ctx = AttackRollContext(
            is_crit=False,
            is_nat_20=False,
            enabled_options=frozenset({"reroll_low"}),
            attack_id="two_handed",
        )
